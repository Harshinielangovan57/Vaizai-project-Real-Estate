...
export const connectWallet = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    if (!window.ethereum) return rejectWithValue('MetaMask is not installed');
...
export const disconnectWallet = createAsyncThunk(
  'wallet/disconnect',
  async () => { localStorage.removeItem('wallet_connected'); }
);
  ...
  extraReducers: (builder) => {
    builder
      .addCase(connectWallet.pending, (state) => {
        state.isConnecting = true; state.error = null;
      })
      ...
      .addCase(connectWallet.rejected, (state, action) => {
        state.isConnecting = false; state.error = action.payload;
      })
      .addCase(disconnectWallet.fulfilled, (state) => {
        state.address = null; state.chainId = null;
        state.balance = '0'; state.isConnected = false;
        state.isCorrectNetwork = false;
      })
